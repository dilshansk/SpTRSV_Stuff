...
void FW_unit_1(tapa::istream<t_DW>& i_valStream,
            tapa::istream<float>& i_bStream,
            tapa::ostream<float>& o_nDiagValBStream,
            tapa::ostream<float>& o_nDiagValToPE,
            tapa::ostream<t_HW>& o_nDiagRowIdxRowEndToPE,
            tapa::ostream<bool>& o_nDiagDummyValStatToPE,
            tapa::ostream<t_DW>& o_xReqVal,
            tapa::ostream<float>& o_diagValBStream,
            unsigned int PEIdx){
  ...
  bool needBSample = false;
  bool diagValBSample = true;
  bool nDiagValBSample = false;
  ...
  for (;;)
  {
    ...
    if (!i_valStream.empty())
    {
      ...
      if ((inputData & 0x1) == 0)
      {
        ...
        if(diagValBSample | (!nDiagValBSample)){   //previously, only diag val row received, so b was sent. Now this is first nonDiagVal after it, so need to send b to accumulator.
          diagValBSample = false;
          nDiagValBSample = true;
          needBSample = true;
        }
        ...
        if(!nDiagValBSample){
          ...
          nDiagValBSample = false;
        }
      }
    }

    if(needBSample){
      float bVal = i_bStream.read();
      if(diagValBSample){
        o_diagValBStream.write(bVal);
        // printf("[DiagValB]::val=%f\n", bVal);
      }

      if(nDiagValBSample){
        o_nDiagValBStream.write(bVal);
        // printf("[nDiagValB]::val=%f\n", bVal);
      }
      needBSample = false;
    }
  }
...
void accumulator(tapa::istream<t_DW>& i_valStream,
  ...
  for (;;)
  {
    ...
    if(((needBSample)) & (!i_bStream.empty()) ){
      i_bStream.try_read(bVal);
      firstBValSampled = true;
      needBSample = false;
    }
    
    if ((!i_valStream.empty()) & (!needBSample))
    {
...
void diagValHandler(tapa::istream<float>& i_diagVal,
  ...
  for (;;)
  {
    ...
    if(i_diagWiatStatus.try_peek(peekWaitStatus)){
      ...
      if ( (peekWaitStatus) & (!i_diagVal.empty()) & (!i_diagRowIdx.empty()) & (!i_sumRespStream.empty())){
        ...
        xSolved = sumRespSample/diagValSample;

        o_rowIdx.write(diagRowIdxSample);
        o_xSolved.write(xSolved);
      }
      else if ( (!peekWaitStatus) & (!i_diagVal.empty()) & (!i_diagRowIdx.empty()) & (!i_diagBStream.empty())){
        i_diagVal.try_read(diagValSample);
        i_diagRowIdx.try_read(diagRowIdxSample);
        i_diagBStream.try_read(sumRespSample);
        i_diagWiatStatus.try_read(waitStatusSample);
...
void sptrsv_PE(tapa::istream<t_WIDE>& i_val_stream,
  ...
  tapa::stream<float, GEN_FIFO_DEPTH> nDiagBStream_FW_to_accum("nDiagBStream_FW_to_accum");
  tapa::stream<float, GEN_FIFO_DEPTH> diagBStream_FW_to_diagHandler("diagBStream_FW_to_diagHandler");
  ...
  tapa::task()
    ...
    .invoke<tapa::detach>(FW_unit_1, valStreams_split_to_FWUnit[1], i_b_stream, nDiagBStream_FW_to_accum, nDiagVal_FW_to_mul[1], nDiagRowIdxRowEnd_FW_to_mul[1], nDiagDummyValStat_to_mul[1], o_x_req_to_mem1, diagBStream_FW_to_diagHandler, PEIdx)
    ...
    .invoke<tapa::detach>(accumulator, sum_cb_to_accum, nDiagBStream_FW_to_accum, sumResp_accum_to_diagHandler)
    .invoke<tapa::detach>(diagValHandler, diagVal_FW_to_diagHandler, diagRowIdx_FW_to_diagHandler, diagWaitStatus_FW_to_diagHandler, sumResp_accum_to_diagHandler, diagBStream_FW_to_diagHandler, o_xSolved_rowIdx, o_xSolved_val, PEIdx);
}
