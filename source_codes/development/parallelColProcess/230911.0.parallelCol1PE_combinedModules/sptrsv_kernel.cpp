...
void accumulator(tapa::istream<t_DW>& i_valStream,
  ...
  {
    #pragma HLS PIPELINE II=1
    #pragma HLS dependence variable=part_sum_val inter RAW true distance=5
    if (!i_valStream.empty())
    {
      ...
      float ps_val = part_sum_val[i];
      t_HW ps_rowIdx = part_sum_rowIdx[i];

      if (rowIdxSample == ps_rowIdx)
      {
        adder_tree_val[i] = ps_val + sum_in_float;
        part_sum_val[i] = ps_val + sum_in_float;
      }
      else{
        adder_tree_val[i] = sum_in_float;
        part_sum_val[i] = sum_in_float;
      }
      adder_tree_rowIdx[i] = rowIdxSample;
      part_sum_rowIdx[i] = rowIdxSample;


      if (rowEndSample==1)  //Row end reached. Invoke adder tree to take the sum
      {
        float valAdder_tree_l0_0 = ((adder_tree_rowIdx[0]==rowIdxSample)?adder_tree_val[0]:0) + ((adder_tree_rowIdx[1]==rowIdxSample)?adder_tree_val[1]:0);
        float valAdder_tree_l0_1 = ((adder_tree_rowIdx[2]==rowIdxSample)?adder_tree_val[2]:0) + ((adder_tree_rowIdx[3]==rowIdxSample)?adder_tree_val[3]:0);
        float valAdder_tree_l0_2 = ((adder_tree_rowIdx[4]==rowIdxSample)?adder_tree_val[4]:0);

        float valAdder_tree_l1_0 = valAdder_tree_l0_0 + valAdder_tree_l0_1;

        nonDiagSumVal = valAdder_tree_l1_0 + valAdder_tree_l0_2;

        o_sumRespStream.write(nonDiagSumVal);
      }
      i = i==4 ? 0 : i+1;
    }
  }
