...
bool contains(const char * arr, const char& ch) {
    bool result = false;
...
void readMatrixCOO(const string& filename, vector<float>& values, vector<int>& rowIdx, vector<int>& colIdx, int& rows, int& cols, int& nnz) {
    ...
    while(1){
        long int prev_pos = ftell(filepointer);
        status = fscanf(filepointer, "%[^\n]\n", buffer);
        if (status!=EOF){  
            if (contains(buffer, '%')){ //still in comment section
                printf("%s\n", buffer);
                continue;
            }
            else{   //Done with comment section
                printf("Done comment section\n");
                fseek(filepointer,prev_pos,SEEK_SET);
                break;
            }
        }
        else{
    ...
    status = fscanf(filepointer, "%d %d %d\n", &rows, &cols, &nnz);
    if (status!=EOF){
        printf("%d %d %d\n", rows, cols, nnz);
    }
    else{
    ...
    double value;
    while(1){
        status = fscanf(filepointer, "%d %d %lf\n", &row, &col, &value);
        if (status!=EOF){  
