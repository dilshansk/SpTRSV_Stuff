...
void preProcessing(int& rows, int& cols, int& nnz, std::vector<float>& csr_values, std::vector<int>& csr_colIdx, std::vector<int>& csr_rowIdx, std::vector<float>& b_vec, std::vector<float> *value_vec, std::vector<int> *colIdx_vec, int *streamsize){
    ...
    for (int i = 0; i < rows; i=i+NU_PE)
    {
        for (int j = 0; j < NU_PE; j++)
        {
            ...
            float b_val = b_vec[i+j];
            int row_size = row_end - row_start;
            
            value_vec[j].push_back(b_val);
            colIdx_vec[j].push_back((i+j));
...
int main(int argc, char** argv)
{
    ...
    preProcessing(rows, cols, nnz, csr_values, csr_colIdx, csr_rowIdx, b_vec, value_vec, colIdx_vec, streamSize);
    printf("Stream 1 size=%d, Stream 2 size=%d, stream 3 size=%d, stream 4 size=%d\n",streamSize[0], streamSize[1], streamSize[2], streamSize[3]);
    ...
    int64_t kernel_time_ns = tapa::invoke(sptrsv_kernel, FLAGS_bitstream, 
            tapa::read_only_mmap<const float>(value_vec[0]), 
            tapa::read_only_mmap<const int>(colIdx_vec[0]), 
            tapa::read_only_mmap<const float>(value_vec[1]), 
            tapa::read_only_mmap<const int>(colIdx_vec[1]), 
            tapa::read_only_mmap<const float>(value_vec[2]), 
            tapa::read_only_mmap<const int>(colIdx_vec[2]), 
            tapa::read_only_mmap<const float>(value_vec[3]), 
            tapa::read_only_mmap<const int>(colIdx_vec[3]),  
            tapa::write_only_mmap<float>(x_vec), 
            rows, streamSize[0], streamSize[1], streamSize[2], streamSize[3]);
